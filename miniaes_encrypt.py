...
def mix_column(string):
    ...
    for i in range(0, len(bits)):
        # [MARCAÇÃO]
        val1 = bits[i].replace('0b', '').zfill(4)
...
def key_addition(step_3, key_matrix):
    # [MARCAÇÃO]
    result = []
    for i in range(len(step_3)):
        # [MARCAÇÃO]
        val_step3 = step_3[i].replace('0b', '').zfill(4)
        val_key = key_matrix[i % len(key_matrix)].replace('0b', '').zfill(4)
        
        xor_result = ""
        for b1, b2 in zip(val_step3, val_key):
            xor_result += '1' if b1 != b2 else '0'
        result.append(xor_result)
    return result

def matrix_nibble_generator(plaintext, key):
    # [MARCAÇÃO]
    ...
    # [MARCAÇÃO]
    step_4 = key_addition(step_3, key_matrix)
